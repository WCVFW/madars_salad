...
-- Create a view for admin subscription summary
...
SELECT 
  ...
  us.meals_delivered,
  us.meals_cancelled,
  us.total_paused_days,
  us.pause_limit_days,
  us.created_at,
  COALESCE(
    (SELECT COUNT(*) FROM public.subscription_days sd 
     WHERE sd.subscription_id = us.id AND sd.meal_taken = true), 
    0
  ) as total_days_taken,
  COALESCE(
    (SELECT COUNT(*) FROM public.subscription_days sd 
     WHERE sd.subscription_id = us.id AND sd.cancelled = true), 
    0
  ) as total_days_cancelled
FROM public.user_subscriptions us
LEFT JOIN public.profiles p ON us.user_id = p.id
LEFT JOIN public.subscription_plans sp ON us.plan_id = sp.id;

-- Add RLS for the admin view
CREATE POLICY "Admins can view subscription summary" 
ON public.admin_subscription_summary 
FOR SELECT 
USING (has_role(auth.uid(), 'admin'::app_role));
...
CREATE TRIGGER trigger_generate_subscription_days
AFTER INSERT OR UPDATE ON public.user_subscriptions
FOR EACH ROW
EXECUTE FUNCTION public.handle_subscription_days_generation();